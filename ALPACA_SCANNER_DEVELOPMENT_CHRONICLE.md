...
| **V1.3d** | **Risk/Reward & Stop-Distance Diagnostics** | **ACCEPTED / FROZEN (SHADOW)** |
...
V1.3d was validated as a separate risk-geometry diagnostic layer consuming frozen V1.3c trigger/zone references. Focused V1.3b/V1.3c/V1.3d validation completed **80/80 PASS**. Live S&P 500 validation covered 116 candidates and Russell 2000 covered 90 candidates; both passed official-layer integrity.

Across both universes, R:R degraded coherently as the hypothetical fill moved from trigger to zone high to maximum fill. The trigger R:R headline is construction-based because T2 is fixed at 2.5R; it is therefore not treated as independent evidence of edge.

No official Candidate Quality, F15, Entry Quality, ranking, bucket, event gate, legacy entry/stop/target field, or trade decision was changed. V1.3d is frozen as **shadow only**. The 0.25 ATR stop buffer, 1.5R/2.5R targets and R:R bands remain provisional and require Backtest + Forward Test + Expectancy evidence before any production authority.

**Decision:** Freeze V1.3d and proceed to **V1.3e — Decision Architecture (READY / WAIT / NO CHASE)**.
